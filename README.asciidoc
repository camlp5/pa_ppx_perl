...
```sh
$ perl -MData::Dumper -e '$x = "#!/foo/bar/buzz  abc  def"; $x =~ m,^#!(\S+)/([^/\s]+),; print Dumper([$&, $1, $2]);'
$VAR1 = [
          '#!/foo/bar/buzz',
          '/foo/bar',
          'buzz'
...
```sh
$ perl -MData::Dumper -e '$x = "abcdaceabc" ; @l = split(/ab?c/, $x); print Dumper([@l]);'
$VAR1 = [
          '',
          'd',
          'e'
...
```sh
$ perl -MData::Dumper -e '$x = "abcdaceabc" ; @l = split(/a(b)?c/, $x); print Dumper([@l]);'
$VAR1 = [
          '',
          'b',
          'd',
...
capture group `2` is *mandatory*.  Then the modifier `exc strings !2`
would produce the result type `string` (and only capture group `2`).
If we wanted capture groups `1`, `3`, we would use the modifier `exc
strings (!1, !3)`, and the result type would be `string * string`.  If
we wanted capture groups `1`, `2`, but not mandatory, the modifier
`strings (1,2)` would give us a result type `(string option * string
option) option` and no exception getting raised.
